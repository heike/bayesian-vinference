...
\svp{Is this thing $B_t$ technically a bayes factor any more? I would think so, but I'm not sure...?}
...
\[
B(p_i) = \int_0^1 B_t(p_i) dc = \int_0^1 \frac{P(p_i > t \mid x, \alpha)}{P(p_i > t \mid \alpha)} dt.
\]
...
<<bayes-factor-investingation, eval = F>>=
...
bf.sim <- 0:10 %>%
  purrr::map_df(.f = function(i) {
    1 %>%
      purrr::map_df(.f = function(j){
        data.frame(
          pic_id = paste(i, j, sep = "."),
          response_no = as.character(c(rep(1, i), sample(2:20, size = (10 - i), replace = T))))
      })
  })
...
counts <- tmp[i, "counts"][[1]][[1]]
probs <- tmp[i, "post_prob"][[1]][[1]]
preprobs <- tmp[i, "pre_prob"][[1]][[1]]
bfactor <- tmp[i, "bfactor"]$bfactor[[1]]$avg
bfs <- tmp[i, "bfactor"]$bfactor[[1]]$factor
dframe <- data.frame(variable = 1:20, Bfactor= round(bfactor,2))
...
tmp$data.picks <- stringr::str_extract(tmp$pic_id, "^\\d{1,}") %>% as.numeric()
  tmp.df <- tmp$bfactor
