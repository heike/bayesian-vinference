...
\centering
<<prior, echo=FALSE, fig.width = 8, fig.height = 8, out.width='\\textwidth'>>=
library(gtools)
# library(reshape2)
library(tidyr)
library(ggplot2)
...
@
\caption{\label{fig:prior}Noninformative prior}
\end{subfigure}
\begin{subfigure}
\centering
<<prior-model2, echo=FALSE, fig.width = 8, fig.height = 8, out.width='\\textwidth'>>=
m <- 20
...
qplot(value, geom="density", data=lpsm, fill=I("grey50"),
  ...
  xlab("Probability to pick plot")
@
\caption{\label{fig:prior-model2}Strong prior}
\end{subfigure}
\caption{Two different Dirichlet($\alpha$) priors for a lineup with 20 panels.}
\end{figure}
...
Figure \ref{fig:bayes-factor-investigation-Null} shows the distribution of Bayes factors of the 19 null plots in each simulated scenario.
...
ggplot() +
  ...
  ggtitle(sprintf("Comparison of Bayesian and Frequentist Lineup Analysis Methods with %d Lineup Evaluations", Npicks)) +
