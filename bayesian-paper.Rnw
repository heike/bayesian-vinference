...
\svp{A flat Dirichlet distribution isn't the same as a Jeffrey's prior. Flat priors are improper, which is probably a good idea to avoid as it makes calculation of Bayes factors that much more of a PITA.}
We can assign an uninformative prior probability to $p$ by using a
%flat
Dirichlet distribution (a
%uniform
single-parameter
distribution over the $(m-1)$ simplex,
with identical marginals for each plot $i$). An uninformative prior is desireable because a priori we assume that the plots are all equally likely to be selected by a participant.
...
K &=& \text{ Total number of evaluations}\\
...
bfactors.fix <- function(alpha, counts) {
  x <- seq(0,1, by=0.001)
  bf <- lapply(1:20, function(i) {
    pre <- pbeta(x, shape1=alpha[i], shape2=sum(alpha)-alpha[i], lower.tail=FALSE)
    post <- pbeta(x, shape1=alpha[i]+counts[i],
                shape2=sum(alpha)-alpha[i] + sum(counts) - counts[i],
                lower.tail=FALSE)
    sum(post)/sum(pre)
...
bfactor <- turk1013.picks[i, "bfactor.fix"]$bfactor.fix[[1]] %>% unlist()
...
bfactor <- turk19.picks[i, "bfactor.fix"]$bfactor.fix[[1]] %>% unlist()
...
bfactor <- turk16.picks[i, "bfactor.fix"]$bfactor.fix[[1]] %>% unlist()
dframe <- data.frame(variable = 1:20, Bfactor= round(bfactor,2))
