...
bfactor <- function(alpha, counts) {
  x <- seq(0,0.9999, by=0.005)
  bf <- lapply(x, function(z) {
    pre <- pbeta(z, shape1=alpha, shape2=sum(alpha)-alpha, 
                           lower.tail=FALSE)
    post <- pbeta(z, shape1=alpha+counts, 
                shape2=sum(alpha)-alpha + sum(counts) - counts,
                lower.tail=FALSE)
    post/pre
  })
  # now get averages
...
bfactor <- turk1013.picks[i, "bfactor"][[1]][[1]]
...
bfactor <- turk19.picks[i, "bfactor"][[1]][[1]]
dframe <- data.frame(variable = 1:20, Bfactor= round(bfactor,2))
...
\hh{XXX knitr question how do I set the number of digits in the output automatically?}

\paragraph{Calculation of the Bayes factor for plot \#$i$}\hfill\newline
\hh{XXX after discussion with Susan: put a prior on the cutoff probability - Uniform, I guess, and either integrate it out or find a mode in the posterior.}
...
bfactor <- turk16.picks[i, "bfactor"][[1]][[1]]
dframe <- data.frame(variable = 1:20, Bfactor= round(bfactor,2))
