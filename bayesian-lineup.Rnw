...
\if0\blind {
  ...
  \author{Susan VanderPlas, Dianne Cook, Christian R\"ottger, Heike Hofmann\thanks{
...
if (!file.exists("data/all_p_values_computed.Rdata"))
...
t10plot_data %>%
  ...
  geom_text(aes(x = -Inf, y = Inf, label =  n),
            hjust = 0, vjust = 1.1,
            size = 8, colour= "grey80", data = counts_turk10 %>% filter(n > 0), inherit.aes = F) +
#  geom_text(aes(x = -Inf, y = Inf, label = sprintf("c[%d] == %.0f", .sample_inner, n)),
#            hjust = -0.2, vjust = 1, parse = T, data = counts_turk10 %>% filter(n > 0), inherit.aes = F) +
