...
Initially, we assume that all plots are equally likely to be selected by a participant, that is, $\bm{\alpha} = \alpha$ ($\alpha_i = \alpha_j$ for all $i,j$), but the concentration parameter allows us to vary this belief.
