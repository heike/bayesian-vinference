...
\section{Lineup Evaluations}\label{sec:scenarios}
% Types of lineups
...
$\alpha$ plays a similar role in modulating the visual p-value calculated from \autoref{eqn:sim-pmf}: lower $\alpha$ values produce a higher visual p-value estimate, and higher $\alpha$ values produce a lower visual p-value estimate.
...
We have already established that the choice of $\alpha$ has a large impact on the visual p-value (\autoref{fig:vis-p-val-sensitivity-initial}), but if we use the estimation method described in the last section, how do the results change in practice? Here, we show 3 different lineups - nonsignificant, marginal, and highly significant, and examine the visual p-values computed under the binomial distribution, the \texttt{vinference} package (with default parameter $\alpha = 1$), and with the $\alpha$ estimated from the experiment which originally included the lineup in question.

<<turk13-alpha-setup, include = F>>=
lineup_res <- filter(studies_sum, study == "turk13", str_detect(pic_name, "filebab6558ba4c4-multiple"))
alpha_val <- lineup_res %>%
  select(study, param_value, test_param) %>%
  unique() %>%
  left_join(studies_alpha_est)
alpha_val <- alpha_val$alpha

target_sel <- filter(lineup_res, obs_plot_location == response_no)$n
K <- sum(lineup_res$n)

ps <- c(
  dbinom(target_sel, K, 1/20),
  vinference::dVsim(target_sel, K, scenario = 3)$scenario3,
  # vis_p_value_orig(target_sel, K),
  vis_p_value(target_sel, K, alpha_val)
) %>%
  round(4)
@
<<turk13-lineup, fig.cap = sprintf("A lineup from \\citet{loyAreYouNormal2015} which was evaluated a total of %d times. Number of selections $c_i$ are shown at the top left of each panel.", sum(counts_turk13$n)), fig.width = 6, fig.height = 6/5*4.5, fig.align='center', out.width = ".66\\textwidth">>=
