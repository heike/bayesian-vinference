...
Intuitively, $\alpha$ controls the proportion of null plots which would have some visually interesting feature: if $\alpha$ is low, that is, $\alpha << 1$, $\theta$ generated by the model would tend to be zero, with one or two nonzero probabilities (that is, one or two of the plots would be significantly more noticable).
