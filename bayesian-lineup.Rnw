...
# t10plot_data <- read_csv("data/Turk10/data/data-1-1-9-20-10-13-20.csv")
#
# counts_turk10 <- studies_sum %>%
#   ungroup() %>%
#   filter(study == "turk10" & pic_name == "filebab65bda535f-multiple.svg") %>%
...
\end{align}
where $C$ is the number of data panel detections and $K$ is the number of independent evaluations of the lineup~\citep{vinference,vanderplas:2017,loyAreYouNormal2015}. A similar method is found in the \texttt{vinference} package, which calculates visual p-values by simulating draws of $\theta$ from a uniform distribution (which corresponds to an assumption that $\alpha=1$) but the more general solution is useful to consider, as we may not actually believe $\theta$ is uniformly distributed over the $(m - 1)$ simplex.
...
\autoref{fig:turk10-lineup} shows a plot which
...
lineup_res <- filter(all_p_val_methods, study == "turk10", str_detect(pic_name, "filebab65bda535f-multiple"))$p_calc[[1]]
@

<<turk10-lineup, fig.cap = sprintf("A lineup from \\citet{loyAreYouNormal2015} which was evaluated a total of %d times. Number of selections $c_i$ are shown at the top left of each panel. The binomial p-value for this model is %0.4f, the simulated vinference p-value is %0.4f, the multinomial p-value with $\\alpha = 1$ is %0.4f, and the multinomial p-value with $\\hat\\alpha = %0.4f$ is %0.4f", sum(counts_turk13$n), lineup_res[1], lineup_res[2], lineup_res[3], filter(all_p_val_methods, study == "turk10", str_detect(pic_name, "filebab65bda535f-multiple"))$alpha, lineup_res[4]), fig.width = 6, fig.height = 6/5*4.5, fig.align='center', out.width = ".66\\textwidth">>=
t10plot_data %>%
  ggplot() +
