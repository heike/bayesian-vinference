...
While visual inference was initially developed to mimic frequentist hypothesis tests with the standard threshold of $p = 0.05$, visual inference itself does not require use of frequentist techniques. In this section, we show one possible framework for visual inference, using a Dirichlet-multinomial distribution to model the probabilities of selecting each panel and a Multinomial distribution to model the observed participant selections. This framework for analysis of visual inference data has been in use as part of the \texttt{vinference} package \citep{vinference} for several years but has not been presented as part of a publication before \svp{I think this is true, but I'm not 100\% sure...}.
