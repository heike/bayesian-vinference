...
\item [Scenario 2] $K$ different sets of null plots are shown to $K$ independent individuals; the same data plot is used in each lineup. This scenario is more similar to Scenario 1 than to Scenario 3, but some dependence is introduced due to the fact that one panel of the lineup (the data panel) is common to all evaluated lineups.
...
Note that even though the interesting null plot effect is found in all scenarios proposed in \autoref{sec:scenarios}, it only becomes a systematic issue in Scenario 3; in Scenarios 1 and 2, the null plots are re-generated with each lineup evaluation, so an interesting null plot evaluated by one individual does not effect the evaluation of other individuals. In Scenario 3, however, the interesting null plots affect all evaluations, and this effect can significantly effect the experiment results.
